...
module.exports = (
  ...
  g.task "#{taskPrefix}python.syntax", ->
    virtualenv "flake8 #{package_dest} tests", venvPath, activateVenv
  g.task "#{taskPrefix}python.complex", ["#{taskPrefix}python.syntax"], ->
    virtualenv "radon cc -nc #{package_dest} tests", venvPath, activateVenv
  g.task "#{taskPrefix}python.mentain", ["#{taskPrefix}python.complex"], ->
    virtualenv "radon mi -nc #{package_dest} tests", venvPath, activateVenv
