  ...
  class Python extends Base {
    ...
    init(g) {
      ...
      g.task(`${this.opts.taskPrefix}python.syntax`, () => this.pyvenv(
        `flake8 --exclude='${excludePatterns.join(',')}' ${this.src} tests`
      ));
      g.task(
        `${this.opts.taskPrefix}python.complex`,
        () => this.pyvenv(
          `radon cc -nc -e '${excludePatterns.join(' ')}' \
          -i '${excludePatterns.join(' ')}' ${this.src} tests`
        )
      );
      g.task(
        `${this.opts.taskPrefix}python.mentain`,
        () => this.pyvenv(
      ...
      g.task(
        ...
        () => this.pyvenv(
          `nosetests ${this.opts.nosetestsArgs.join(' ')} tests`
        )
      );
      g.task(
        `${this.opts.taskPrefix}python.unittest`,
        () => this.pyvenv(
          `python -m unittest ${this.opts.unittestArgs.join(' ')} tests`
        )
      );
      g.task(`${this.opts.taskPrefix}python.tox`, () => this.pyvenv('tox'));
    }
  }
